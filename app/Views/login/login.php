			...
			<div class="container container-tight my-5 px-lg-5">
				...
				<div class="mb-3">
					...
					<select class="form-select" id="cmbempresa" name="cmbempresa">
						<?php foreach ($empresa as $empresas): ?>
							<option value="<?= esc($empresas['idempresa']); ?>">
								<?= esc($empresas['descripcion']); ?>
							</option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<div class="mb-3">
					...
					<select class="form-select" id="cmbusuario" name="cmbusuario">
						<?php foreach ($usuarios as $usuario): ?>
							<option value="<?= esc($usuario['idusuarios']); ?>">
								<?= esc($usuario['usuario']); ?>
							</option>
						<?php endforeach; ?>
					</select>
				...
				<div class="mb-2">
					...
					<div class="input-group input-group-flat">
						<input type="password" class="form-control" id="txtpassword" placeholder="Your password" autocomplete="off">
				...
				<div class="form-footer">
					<button class="btn btn-primary w-100" onclick="loguear_sistema()">Iniciar sesión</button>
				</div>
				<div class="text-center text-secondary mt-3">
					Todos los derechos reservados <a href="./registro.php" tabindex="-1">IMB</a>
				</div>
			</div>
		</div>
		<div class="col-12 col-lg-6 col-xl-8 d-none d-lg-block">
			<!-- Photo -->
			<div class="bg-cover h-100 min-vh-100" style="background-image: url('<?= base_url('public/static/photos/ciberseguridad.jpg') ?>');"></div>
		</div>
	</div>
	<!-- Libs JS -->
	<!-- Tabler Core -->
	<script src="<?= base_url('public/dist/js/tabler.min.js?1738096682') ?>"></script>
	<script src="<?= base_url('public/dist/js/demo.min.js?1738096682') ?>"></script>
